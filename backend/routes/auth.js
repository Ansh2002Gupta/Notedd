...
router.post("/createuser", [body("first_name", "Enter a valid first name").isLength({ min: 3 }), body("last_name", "Enter a valid last name").isLength({ min: 3 }), body("username", "Enter a valid username").isLength({ min: 3 }), body("email", "Enter a valid email").isEmail(), body("password", "Password must be atleast 5 characters long").exists().isLength({ min: 5 })], async (req, res) => {
  let success = false;

  //if there are errors then return bad request and the errors
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ success, error: errors.array()[0].msg });

  //check whether the user with this email already exists or not.
  try {
    let user = await User.findOne({ email: req.body.email });
    if (user) return res.status(400).json({ success, error: "Sorry! This user already exists." });
    ...
    res.status(500).send("Some error has occured | success: " + success);
  }
});

//creating second endpoint.
//ROUTE 2: Create a user login endpoint, using POST: "/api/auth/login". No need of user login, signup.
router.post("/login", [body("email", "Enter valid email address").isEmail(), body("password", "Password can't be blank and should be atleast 5 characters long").exists().isLength({ min: 5 })], async (req, res) => {
  let success = false;
  ...
  try {
    ...
    if (!user) return res.status(400).json({ success, error: "Please enter valid mail credentials" });

    //checking the validity of the password entered by the user and the database password using bcrypt.
    const isCorrectPassword = await bcrypt.compare(password, user.password);
    if (!isCorrectPassword) return res.status(400).json({ success, error: "Please enter valid password credentials" });
