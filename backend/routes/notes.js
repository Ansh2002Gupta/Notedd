...
router.post("/addnotes", fetchuser, [body("title", "Title should be atleast 3 character long").isLength({ min: 3 }), body("description", "Description should be atleat 5 characters long.").isLength({ min: 5 }), body("tag", "tag should not be empty").exists()], async (req, res) => {
  let success = false;
...
router.put("/updatenote/:id", fetchuser, async (req, res) => {
  ...
  try {
    ...
    if (note.user.toString() != req.user.id) return res.status(401).send("You aren't authorized to do so!");
